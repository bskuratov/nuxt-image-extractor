...
module.exports = function Extract(moduleOptions) {
  ...
  async function process(page) {
    ...
    for (const match of matches) {
      ...
      if (responsiveImageMatches && responsiveImageMatches.length) {
        matchURLStrings = match[0]
          .split(responsiveImagesRegex)
    ...
    consola.info(
      `${page.route}: nuxt-image-extractor is replacing ${urls.length} images with local copies`
    );
  ...
  async function replacePayloadImageLinks(payload, urls) {
    ...
    consola.info(
      `nuxt-image-extractor replaced ${count} image links in this payload`
    );
    return payload;
  }
};
...
async function saveRemoteImage(url, path) {
  ...
  // This handles both Node and web streams
  return await streamPipeline(res.body, fs.createWriteStream(path));
}
